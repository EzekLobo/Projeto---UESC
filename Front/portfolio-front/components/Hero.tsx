...
export default function Hero({ perfil }: { perfil: Perfil }) {
  ...
  // Função para garantir que a URL da mídia seja absoluta
  ...
  const fotoUrl = getFullUrl(perfil?.foto);
  const cvUrl = getFullUrl(perfil?.cv);
          ...
          <div className="flex flex-nowrap gap-3 pt-2 justify-center lg:justify-start">
            {perfil?.cv && (
              <a 
                ...
                download 
                className="group flex items-center justify-center gap-3 bg-brand-red text-white hover:bg-red-600 px-4 py-3.5 md:px-8 rounded-xl font-bold transition-all shadow-[0_0_20px_-5px_rgba(255,51,51,0.3)] hover:shadow-[0_0_20px_-5px_rgba(255,51,51,0.5)]"
              >
